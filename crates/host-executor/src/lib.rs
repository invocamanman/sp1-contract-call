...
use sp1_cc_client_executor::{
    io::EVMStateSketch, new_evm, new_evm_calldata, ContractInput, ContractInputCalldata,
};
...
impl<T: Transport + Clone, P: Provider<T, AnyNetwork> + Clone> HostExecutor<T, P> {
    ...
    // Executes the smart contract call with the given [`ContractInput`].
    pub async fn executeCalldata(&mut self, call: ContractInputCalldata) -> eyre::Result<Bytes> {
        let cache_db = CacheDB::new(&self.rpc_db);
        let mut evm = new_evm_calldata(cache_db, &self.header, U256::ZERO, &call);
        let output = evm.transact()?;
